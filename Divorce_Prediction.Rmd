...
Dans cette section nous procéderons à une grande...
...
for (i in 0:4) {
  ...
  for (j in 1:2) {
    ...
    hist(
      ...
      main = var_name,
      col = "#5DADE2",
      prob = TRUE,            # Pour afficher la densité et non la fréquence
      cex.main = 0.9,
      cex.axis = 0.9,
      las = 1,
      frame.plot = FALSE,
...
premier_temps <- c("Probabilité que le mariage dure ≥ t",
                   "Risque cumulé de divorce jusqu’à t",
                   "Risque instantané de divorce à t")
second_temps <- c("Probabilité de rester marié si marié à l’âge t",
...
kable(tableau, "html", caption = "📊 Interprétation des fonctions de survie") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = FALSE) %>%
...
## - Analyse de la Durée de marriage avant divorce 
...
plot(NA, xlim = c(1, max(t_values)), ylim = c(0,5),
     xlab = "Durée du mariage (années)",
     ylab = "H(t) = Risque cumulée",
     main = "Comparaison fonction de risque cumulé")
...
# hazard approximatif à chaque intervalle
...
plot(time_hazard, hazard, type = "s",
     xlab = "Durée du mariage (années)",
     ylab = "Risque instantané",
     main = "Fonction de risque estimée à partir de Cox",
     lwd = 2, col = "red")
grid()
...
plot(smooth_h, type="l", lwd=2, col="blue",
     xlab="Durée du mariage (années)",
     ylab="Risque instantané",
     main="Fonction de risque lissée à partir de Cox")
grid()
