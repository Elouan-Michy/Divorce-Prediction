...
# Tableau combiné Nelson-Aalen + Breslow
...
kable(risk_methods, "html", escape = FALSE,
  ...
  # Style colonne Méthode (différencié)
...
event_no_cens <- rep(1, length(time))  # tout le monde a eu l'événement
...
# --- Tracé avec intervalles de confiance ---
ggplot(df_merge, aes(x = time)) +
  # ribbons IC
  geom_ribbon(aes(ymin = H_Breslow_lower, ymax = H_Breslow_upper),
              fill = "blue", alpha = 0.15) +
...
```{r education_level 16, echo=FALSE, fig.align='center'}
...
```{r survival_without_parameters_8, echo=FALSE, fig.align='center'}
# Assurer que education_level est bien un facteur avec des niveaux lisibles
