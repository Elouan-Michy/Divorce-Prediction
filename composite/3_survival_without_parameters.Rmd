...
```
# Estimation du hazard instantané avec kernel smoothing
```{r survival_without_parameters_2, echo = FALSE, fig.align='center'}

time <- data.brute$marriage_duration_years
event <- data.brute$divorced

# Appel muhaz avec les noms explicites
haz_fit <- muhaz(times = time,
                 bw.method = "global",
                 bw = 2)  # largeur de lissage

plot(haz_fit, main="Hazard instantané", xlab="Durée du mariage (années)", ylab="Risque instantané")
